...
def pad_image(image):
    ...
    if h > w:
        ...
        image = cv2.resize(image, (w1, h1))
    # 计算需要填充的宽度和高度
    top, bottom, left, right = (256 - h1) // 2, (256 - h1) - ((256 - h1) // 2), (256- w1) // 2, (256 - w1) - ((256 - w1) // 2)
    # 添加边框
    pimage = cv2.copyMakeBorder(image, top, bottom, left, right, cv2.BORDER_CONSTANT, value=[0, 0, 0])
    return pimage
...
def calculate_frechet_distance(mu1, sigma1, mu2, sigma2, eps=1e-6):
    ...
    # print("[Shape]", sigma1.shape, " ", sigma2.shape)
...
def calculate_fid(images1, images2, use_multiprocessing, batch_size):
    ...
    Returns:
        FID (scalar)
    """
    # print("[Image]", images1.shape, " ", images2.shape)
...
def load_images(path):
    ...
    final_images = np.zeros((len(image_paths), 256, 256, 3), dtype=first_image.dtype)
...
if __name__ == "__main__":
    ...
    parser = OptionParser()
    parser.add_option("--p1", "--path1", dest="path1", default="",
                      help="Path to directory containing the real images")
    parser.add_option("--p2", "--path2", dest="path2", default="",
    ...
    for artist in artists:
        path1 = "exps_attn1/{}/generated_images/".format(artist)
        path2 = "data/test_set/{}".format(artist)
        path3 = "dataset_fake/{}".format(artist)
        path4 = "data/cross-set/{}".format(artist)
        # print(path1)
        # print(path2)
        images1 = load_images(path2)
        images2 = load_images(path4)
        fid_value = calculate_fid(images1, images2, options.use_multiprocessing, options.batch_size)
        print(artist, fid_value)